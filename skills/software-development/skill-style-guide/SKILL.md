...
description: "LLM-first skill authoring standards."
version: 1.0.0
...
LLM-first skill authoring standards. A skill is a runtime instruction contract for an LLM, not human-facing documentation.
...
Every `SKILL.md` MUST follow this order:

1. **Frontmatter** — complete metadata for discovery
...
- Include complete `name`, `description`, `license`, `metadata.author`, `metadata.version`
...
- Hard maximum: **1000 tokens**. Move examples, schemas, background into `assets/` or `references/`
...
- Use `assets/` for templates, schemas, fixtures, generated examples
- Use `references/` for local docs that explain concepts or edge cases
- Keep references stable and relative to the skill directory
