...
> **División de trabajo por herramienta:** **DEFINIR = Hermes** (organiza, define arquitectura/specs/prompts; **solo crea los `.md` de definición — no ejecuta código ni scaffold sin pedido explícito**); **SYNC/DEPLOY = Hermes** (commit/push con evidencia, sync Obsidian, tareas de deploy) → **ORQUESTAR = `opencode plan`** (plan técnico read-only) → **EJECUTAR = `opencode build`** (implementa) → **TESTEAR = `opencode test`** (QA + seguridad, reporta con evidencia).
...
- **Hermes**: **DEFINE** (organiza; crea los `.md` de specs/prompts) + **SYNC/DEPLOY** (commit/push con evidencia, Obsidian, deploy) — no ejecuta código sin pedido explícito; en revisiones aplica `app-quality-gates`
